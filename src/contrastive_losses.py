...
class MultiPosConLoss(nn.Module):
    ...
    def forward(self, features, labels_list):
        feats = features # torch.cat(torch.unbind(features, dim=1), dim=0) #features # outputs['feats']    # feats shape: [B, D]
        labels = labels_list # outputs['labels']    # labels shape: [B]
        ...
        # print(feats.T.shape)
        # print(feats.shape, all_feats.shape)
        ...
        # consider removing image loss for simplicity
        return loss #{'loss': loss, 'image_loss': loss}
