...
class ConClassGraphTrainer:
    ...
    def __init__(self,
        ...
        embeddings_tr, labels_tr, metadata_tr, node_indices_tr = graph_data.extract_embeddings(self.train_loader)
        graph_data_tr = graph_data.construct_graph(embeddings_tr, metadata_tr, labels_tr, node_indices_tr, dist=distance_threshold)

        embeddings_val, labels_val, metadata_val, node_indices_val = graph_data.extract_embeddings(self.val_loader)
        graph_data_val = graph_data.construct_graph(embeddings_val, metadata_val, labels_val, node_indices_val, dist=distance_threshold)

        kwargs = {'batch_size': 512, 'num_workers': 4} # , 'persistent_workers': True
        self.graph_train_loader = NeighborLoader(graph_data_tr, num_neighbors=[5], shuffle=True, **kwargs)
        self.graph_val_loader = NeighborLoader(graph_data_val, num_neighbors=[5], shuffle=False, **kwargs)
    ...
    def train_epoch(self, train_loader, encoder_model, classifier, criterion, optimizer, epoch): # , opt
        ...
        # for idx, (images, labels) in enumerate(train_loader):
            ...
            # Then use the batch coordinates to construct the corresponding graph of the batch?
            ...
            output = classifier(batch.x, batch.edge_index)[:batch_size].to(device=self.device)
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for idx, batch in enumerate(self.graph_val_loader):
    ...
    def train(self, num_epochs: int, early_stopping_patience: int = 10) -> Dict[str, List]:
        ...
        for epoch in range(num_epochs):
            ...
            # Training
            train_loss = self.train_epoch(self.graph_train_loader, self.encoder_model, self.classifier, self.criterion, self.optimizer, epoch + 1) # , train_acc 
